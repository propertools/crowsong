...
| `tools/mnemonic/gloss_twist.py` | Gloss Layer — base-52 key-derived radix re-encoding for non-Latin scripts. Resolves structural CCL entropy limitation for Arabic, CJK, Hangul, Devanagari, Hebrew, Thai. One prime, two schedules (forward=CCL, reversed=gloss). Entropy gains: Chinese +1.36, Korean +1.30, Japanese +1.13, Arabic +0.65 bits/token vs CCL3 alone. See `docs/entropy-analysis.md`. |
| Gloss layer (`gloss_twist.py`) | Implemented. Radix re-encoding of high-codepoint scripts into key-derived base-52 Latin alphabet before CCL. One prime, two schedules: reversed digits drive gloss permutation, forward digits drive CCL. Entropy gains: Chinese +1.36, Korean +1.30, Japanese +1.13, Arabic +0.65 bits/token. See `docs/mnemonic/gloss-README.md`. |
| SHA256 headers for constants, sequences, and primes | Unify the self-describing header format across all cached numeric corpora (`docs/constants/`, `docs/sequences/`, `docs/primes/`) to match the `docs/texts/` pattern — SHA256 declared in header, body verified independently. Operators can then confirm they are using identical inputs to all other operators, and the verification tooling is uniform across the full corpus. See design note below. |
...
## SHA256 headers for numeric corpora — design note

The `docs/texts/` corpus uses a self-describing plain-text header format:

```
# SHA256:    <hex of body>
```

The SHA256 is computed over the body only (after Gutenberg header/footer
stripping and CRLF normalisation). Verification recomputes and checks
against the declared hash. This means:

- Any operator can verify their cached copy matches any other operator's
- The hash is stable across platforms and Python versions
- The verification tool is trivial (one SHA256 call)
- The file is self-describing without external metadata

The same pattern should be applied uniformly to:

**`docs/constants/` — named mathematical constants**

Each `pi-10000.txt`, `e-10000.txt` etc. already carries a SHA256 in its
header (generated by `tools/constants/constants.py`). Verify that the
format and verification path match the `docs/texts/` pattern exactly.
If not, normalise.

**`docs/sequences/` — OEIS sequence mirror**

Each `A000796.txt` etc. already carries a SHA256 in its header
(generated by `tools/sequences/sequences.py`). Same normalisation check.

**`docs/primes/` — prime quick-reference tables**

The primes file (`docs/primes/primes-10000.txt`) generated by
`scripts/generate_numbers.sh` may not carry a SHA256 header. Add one,
using the same format.

**The goal:** a single verification pattern across the full corpus:

```bash
python tools/constants/constants.py verify pi
python tools/sequences/sequences.py verify A000796
python tools/texts/texts.py verify shakespeare
# All produce: PASS <id> (<N> chars/terms)
# All check SHA256 declared in header against SHA256 of body
```

Operators comparing corpus hashes can confirm identical inputs with a
single command per file. This matters for reproducibility of verse-to-prime
derivation when the named sequence or constant is used as a mnemonic
anchor or IV source.

**Implementation:** audit the header format in all three tools against
the `texts.py` reference implementation. Normalise CRLF handling and
body extraction to match. No new files needed — this is a consistency
pass across existing tools.

**Status:** design intent recorded. Audit and normalisation pending.
