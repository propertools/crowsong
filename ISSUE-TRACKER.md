...
**Fix:** Add `parser.add_argument('--version', action='version', version='%(prog)s 1.0')` to `build_parser()`.
...
**Description:** `text_type = bytes if False else (str if not PY2 else type(u""))` — the `bytes if False` branch is unreachable and confuses
readers.
...
`gutenberg_id: None`), `make_text_file()` renders `Project Gutenberg #None` and `https://www.gutenberg.org/ebooks/None` in the header.
...
**Description:** The howto references `python tools/udhr/udhr.py analyse --all`. The `analyse` subcommand does not exist in `udhr.py`.
...
1. `prime_twist.py`: implement `prime-chain/1` schedule
   (see PRIME-CHAIN-001)
1. `w3-entropy-bench.sh`: add Gloss-W3 + prime-chain + mod3 pipeline
...
## scripts/crowsong-backup.sh

### BACKUP-001 — Fork discovery does not recurse into forks-of-forks
...
**Description:** `crowsong-backup.sh` discovers forks via the GitHub
API endpoint `/repos/{owner}/{repo}/forks`, which returns only
first-generation forks — direct forks of `propertools/crowsong`.
If someone forks a fork (e.g. forks `trey-darley/crowsong` rather
than `propertools/crowsong`), that second-generation fork is invisible
to the script and will not be mirrored.

`git clone --mirror` and `git remote update --prune` are correct —
they capture all branches, tags, and refs from every repo they do
reach. The gap is only in fork *discovery*, not in mirroring depth.

The fork tree is currently shallow so this is not urgent. If the
project attracts contributors who fork forks, signal could be lost.

**Fix:** Add `--recursive-forks` flag. For each discovered fork,
call `/repos/{owner}/{repo}/forks` recursively and accumulate all
reachable nodes in the fork tree. Deduplicate by `full_name` before
mirroring. Add a depth limit (e.g. `--max-fork-depth 3`) to prevent
runaway recursion on pathological fork trees.

**Workaround:** operators who know of a second-generation fork can
pass `--upstream owner/crowsong` to run the script a second time
with that fork as the upstream, which will discover and mirror its
own children.